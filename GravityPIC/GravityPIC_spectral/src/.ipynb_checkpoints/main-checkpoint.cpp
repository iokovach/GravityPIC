...
void run_espic ()
{
    ...
    int max_level, n_cell, max_grid_size, n_buffer, max_step, boxsize, bc;
    Real dt;
    std::string ic_file;
    ...
    //but higher levels cannot be periodic
    int is_periodic_hl[AMREX_SPACEDIM];
    for (int i = 0; i < AMREX_SPACEDIM; i++) {
        is_periodic_hl[i] = 0;
    }
    ...
    for (int lev = 1; lev < num_levels; ++lev) {
        geom[lev].define(amrex::refine(geom[lev-1].Domain(), rr[lev-1]),
                         &real_box, CoordSys::cartesian, is_periodic_hl);
    }

    // make grids for each level - level 1 is nested inside level 0
    // this specifically defines a static higher refinement region -- not adaptive at all!!!
    ...
    for (int step = 0; step <= max_step; ++step) {
        ...
        Real mean_rho = rhs[0]->sum_unique(0, false, geom[0].periodicity()) / geom[0].Domain().numPts();  
        
        rhs[0]->plus(-mean_rho, 0, 1);

        //amrex::Print() << "mean rho" << rhs[0]->sum_unique(0, false, geom[0].periodicity()) / geom[0].Domain().numPts() << std::endl;
        
        FieldSolver::computePhi(GetVecOfConstPtrs(rhs), GetVecOfPtrs(phi),
                                grids, dm, geom,
                                GetVecOfConstPtrs(masks), bc);
        
        //amrex::Print() << "mean phi" << phi[0]->sum_unique(0, false, geom[0].periodicity()) / geom[0].Domain().numPts() << std::endl;
